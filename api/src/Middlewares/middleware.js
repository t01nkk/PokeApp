...
const createFromApi = async function () {
  try {
    ...
    if (pokemonsInDb.length === count) {
      // comparo, si son iguales, quiere decir que en la base de datos solo hay pokemones creados por el usuario
      ...
      for (var i = 0; i < apiPoke.length; i++) {
        ...
        let typeDb = await Types.findAll({
          where: { name: apiPoke[i].types[0].name },
        })
        newPoke.addType(typeDb)
        if (apiPoke[i].types[1]) {
          let typeDb2 = await Types.findAll({
            where: { name: apiPoke[i].types[1].name },
          })
...
const filters = async (typeFilter, orderBy) => {
  try {
    ...
    if (filterType.length) {
      // Checkeo si encontró algo
      var allTypes = []
      for (var i = 0; i < filterType.length; i++) {
        allTypes.push(
          await Pokemon.findByPk(filterType[i].id, {
            include: {
              model: Types,
              attributes: ['name'],
              through: { attributes: [] },
      ...
      if (orderBy) return setOrder(allTypes, orderBy)
      return allTypes
    }
  } catch (err) {
    console.log(err.message)
