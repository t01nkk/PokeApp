...
router.get('/', async (req, res) => {
  ...
  try {
    if (created !== 'default' && created == true) {
      const createdInDb = await Pokemon.findAll({
        where: { createdDb: created },
      })
    ...
    if (typeFilter !== 'default' && typeFilter) {
      const filteredPoke = await filters(
        typeFilter,
        order !== 'default' && order ? order : null,
      )
...
router.post('/create', async (req, res) => {
  ...
  if (!arrType.length) {
    return res.status(400).json({ info: `Choose at least one type` })
  }
